...
//    private static final String DATA_URL = "http://download.tensorflow.org/example_images/flower_photos.tgz";
    private static final String MAMMO_TRAIN_DIR =" " ;  //was DATA_DIR + "/flower_photos"
    ...
    private static final int numClasses = 2 ;  //was 5

    private static ParentPathLabelGenerator labelMaker = new ParentPathLabelGenerator();
    public static InputSplit trainData,testData, valData;
...
//    public static InputSplit getValData() throws IOException {
//        return valData;
//
//    }

    public static void setupTrainTest(int batchSizeArg, String MAMMO_TRAIN_DIR, String MAMMO_TEST_DIR, int valFraction ) throws IOException {//should be removed
        batchSize = batchSizeArg;
        File trainDir = new File(MAMMO_TRAIN_DIR);
        FileSplit filesInDir = new FileSplit(trainDir, allowedExtensions, rng);
        BalancedPathFilter pathFilter = new BalancedPathFilter(rng, allowedExtensions, labelMaker);

        InputSplit[] filesInDirSplit = filesInDir.sample(pathFilter, 100-valFraction, valFraction);
        trainData = filesInDirSplit[0];
        valData = filesInDirSplit[1];

        File testDir = new File(MAMMO_TEST_DIR);
        filesInDir = new FileSplit(testDir, allowedExtensions, rng);
